...
public abstract class PullToRefreshBase2<T extends View> extends LinearLayout implements IPullToRefresh2<T> {
    /**
     * 定义了下拉刷新和上拉加载更多的接口。
     *
     * @author Li Hong
    ...
    public interface OnRefreshListener<V extends View> {
         ...
         */
        void onPullDownToRefresh(final PullToRefreshBase2<V> refreshView);
         ...
         * @param refreshView 刷新的View
         */
        void onPullUpToRefresh(final PullToRefreshBase2<V> refreshView);
    }
    ...
    /**下拉的状态*/
    private ILoadingLayout2.State mPullDownState = ILoadingLayout2.State.NONE;
    /**上拉的状态*/
    private ILoadingLayout2.State mPullUpState = ILoadingLayout2.State.NONE;
    /**可以下拉刷新的View*/
            ...
            case MotionEvent.ACTION_UP:
                if (mIsHandledTouchEvent) {
                    ...
                    if (isReadyForPullDown()) {
                        // 调用刷新
                        if (mPullRefreshEnabled && (mPullDownState == ILoadingLayout2.State.RELEASE_TO_REFRESH)) {
                            startRefreshing();
                            handled = true;
                        }
                        resetHeaderLayout();
                        ...
                        // 加载更多
                        if (isPullLoadEnabled() && (mPullUpState == ILoadingLayout2.State.RELEASE_TO_REFRESH)) {
                            startLoading();
                            handled = true;
                        }
                        resetFooterLayout();
    ...
    public void onPullDownRefreshComplete() {
        if (isPullRefreshing()) {
            mPullDownState = ILoadingLayout2.State.RESET;
            onStateChanged(ILoadingLayout2.State.RESET, true);
            ...
            postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    mHeaderLayout.setState(ILoadingLayout2.State.RESET);
                }
    ...
    public void onPullUpRefreshComplete() {
        if (isPullLoading()) {
            mPullUpState = ILoadingLayout2.State.RESET;
            onStateChanged(ILoadingLayout2.State.RESET, false);

            postDelayed(new Runnable() {
                @Override
                public void run() {
                    setInterceptTouchEventEnabled(true);
                    mFooterLayout.setState(ILoadingLayout2.State.RESET);
                }
            }, getSmoothScrollDuration());
     ...
     */
    protected LoadingLayout2 createHeaderLoadingLayout(Context context, AttributeSet attrs) {
     ...
     * @return LoadingLayout对象
     */
    protected LoadingLayout2 createFooterLoadingLayout(Context context, AttributeSet attrs) {
    ...
    protected void addHeaderAndFooter(Context context) {
        ...
        final LoadingLayout2 headerLayout = mHeaderLayout;
        final LoadingLayout2 footerLayout = mFooterLayout;
    ...
    protected void pullHeaderLayout(float delta) {
        ...
        if (isPullRefreshEnabled() && !isPullRefreshing()) {
            if (scrollY > mHeaderHeight) {
                mPullDownState = ILoadingLayout2.State.RELEASE_TO_REFRESH;
            } else {
                mPullDownState = ILoadingLayout2.State.PULL_TO_REFRESH;
            }
    ...
    protected void pullFooterLayout(float delta) {
        ...
        if (isPullLoadEnabled() && !isPullLoading()) {
            if (scrollY > mFooterHeight) {
                mPullUpState = ILoadingLayout2.State.RELEASE_TO_REFRESH;
            } else {
                mPullUpState = ILoadingLayout2.State.PULL_TO_REFRESH;
            }
    ...
    protected boolean isPullRefreshing() {
        return (mPullDownState == ILoadingLayout2.State.REFRESHING);
    }
    ...
    protected boolean isPullLoading() {
        return (mPullUpState == ILoadingLayout2.State.REFRESHING);
    }
    ...
    protected void startRefreshing() {
        ...
        mPullDownState = ILoadingLayout2.State.REFRESHING;
        onStateChanged(ILoadingLayout2.State.REFRESHING, true);

        if (null != mHeaderLayout) {
            mHeaderLayout.setState(ILoadingLayout2.State.REFRESHING);
        }
    ...
    protected void startLoading() {
        ...
        mPullUpState = ILoadingLayout2.State.REFRESHING;
        onStateChanged(ILoadingLayout2.State.REFRESHING, false);

        if (null != mFooterLayout) {
            mFooterLayout.setState(ILoadingLayout2.State.REFRESHING);
        }
     ...
     */
    protected void onStateChanged(ILoadingLayout2.State state, boolean isPullDown) {
