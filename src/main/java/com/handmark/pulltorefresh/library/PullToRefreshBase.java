...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	/**
	 * 用来控制仅仅是否保留反弹效果，而不显示下拉刷新文本提示
	 */
	private boolean isShowHeaderText = true;
	/**
     * 用来控制仅仅是否保留反弹效果，而不显示下拉刷新文本提示
     */
	private boolean isShowFooterText = true;
	...
	public boolean isShowHeaderText(){
        return isShowHeaderText;
    }
	
	public boolean isShowFooterText(){
	    return isShowFooterText;
	}

	public void setShowFooterText(boolean showFooterText) {
		isShowFooterText = showFooterText;
	}
	...
	final void setState(State state, final boolean... params) {
		...
		if (DEBUG) {
			...
			Log.d(LOG_TAG, "mCurrentMode = " + mCurrentMode);
		}

		switch (mCurrentMode) {
            case PULL_FROM_END:
                if(isShowFooterText)
                    mFooterLayout.setVisibilityInnerLayout(View.VISIBLE);
                else
                    mFooterLayout.setVisibility(View.INVISIBLE);
                break;
            case PULL_FROM_START:
                if(isShowHeaderText)
                    mHeaderLayout.setVisibilityInnerLayout(View.VISIBLE);
                else
                    mHeaderLayout.setVisibilityInnerLayout(View.INVISIBLE);
                break;
            default:
                // NO-OP
                break;
		...
		switch (mState) {
			case RESET://放手重置
				onReset();
				break;
			case PULL_TO_REFRESH://提示“下拉刷新”
				onPullToRefresh();
				break;
			case RELEASE_TO_REFRESH://提示“释放刷新”
				onReleaseToRefresh();
				break;
			case REFRESHING:
	...
	protected void onReset() {
		...
		mFooterLayout.reset();
		if(!isShowFooterText)
		    mFooterLayout.setVisibilityInnerLayout(View.INVISIBLE);
	...
	private void init(Context context, AttributeSet attrs) {
		...
		isShowHeaderText = a.getBoolean(R.styleable.PullToRefresh_ptrShowHeaderText, true);
		
		isShowFooterText = a.getBoolean(R.styleable.PullToRefresh_ptrShowFooterText, true);
	...
	public void updateTitle(){
		getHeaderLayout().updateHeadText();
	//	mHeaderLoadingView.updateText();
		getFooterLayout().updateFootText();
}
}
