...
public class PullToRefreshPinnedHeaderListView extends PullToRefreshAdapterViewBase<PinnedHeaderListView> {
	...
	private void init() {
		...
		getRefreshableView().setFastScrollEnabled(false);
		getRefreshableView().setFadingEdgeLength(0);
		getRefreshableView().setSmoothScrollbarEnabled(true);
		getRefreshableView().setCacheColorHint(0x00000000);
		getRefreshableView().setDivider(new ColorDrawable(0xff1f262e));// abs__action_bar_background_color
		getRefreshableView().setDividerHeight(1);
		getRefreshableView().setOverScrollMode(OVER_SCROLL_NEVER);//去掉滑动时顶部底部产生的阴影
		// 默认去掉分割线
		...
		getRefreshableView().setHorizontalFadingEdgeEnabled(false);
		getRefreshableView().setVerticalFadingEdgeEnabled(false);
	}
